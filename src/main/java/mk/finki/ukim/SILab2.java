...
public class SILab2 {
    ...
    public static boolean function (User user, List<User> allUsers) { //1
        if (user==null || user.getPassword()==null || user.getEmail()==null){ //2 //edno po drugo posle bool odi 2kata
        ...
        if (user.getEmail().contains("@") && user.getEmail().contains(".")) { //7
            same = 0;//8
            for (int i=0;i<allUsers.size();i++) { //9.0 9.1 9.2
        ...
        String specialCharacters="!#$%&'()*+,-./:;<=>?@[]^_`{|}"; //16 (se izvrsuvaat edno po drugo odma)
        String password = user.getPassword(); //17
        String passwordLower = password.toLowerCase(); //18 //16,17,18
        ...
        if (passwordLower.contains(user.getUsername().toLowerCase()) || password.length()<8) { //19
            ...
            if (!passwordLower.contains(" ")) { //21
                for (int i = 0; i < specialCharacters.length(); i++) { //22.0 - 22.1 - 22.2
